...
export default function CountriesPage() {
  ...
  return (
    <main>
      <div className="page-hero"><div className="shell"><div className="eyebrow">COUNTRY INDEX</div><h1>国家分类</h1><p>按国家与经济体浏览演示资讯，建立清晰的区域索引。</p></div></div>
        ...
        {countries.map((country) => {
          ...
          return (
            <Link className="directory-card" href={`/countries/${encodeURIComponent(country)}`} key={country}>
              <span>COUNTRY</span><h2>{country}</h2><p>{count} 条演示资讯 · 查看国家档案 →</p>
            </Link>
          );
        })}
      </section>
